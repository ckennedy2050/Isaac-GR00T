...
class Gr00tN1d7(PreTrainedModel):
    ...
    def __init__(
        ...
        if getattr(config, "use_depth_encoder", False):
            self.depth_encoder = DepthEncoder(output_dim=config.depth_encoder_output_dim)
            logging.info("Loaded depth encoder")
            if not getattr(config, "tune_depth_encoder", True):
                logging.info("Freezing depth encoder")
